...
from applications.task.serialziers import (
    BatchUpdateId3Serializer,
    FetchId3ByTitleSerializer,
    FetchLlyricSerializer,
    FileListSerializer,
    Id3Serializer,
    TaskSerializer,
    TidyFolderSerializer,
    TranslationLycSerializer,
    UpdateId3Serializer,
...
from applications.task.tasks import (
    batch_auto_tag_task,
    clear_music,
    full_scan_folder,
    scan,
...
class TaskViewSets(GenericViewSet):
    ...
    def file_list(self, request, *args, **kwargs):
        ...
        task_map = dict(
            Task.objects.filter(parent_path=file_path).values_list("filename", "state")
        )
        ...
        if "name" in sorted_fields:
            children_data = sorted(
                children_data,
                key=lambda x: x.get("name").encode("gbk", "ignore"),
                reverse=False,
            )
        if "update_time" in sorted_fields:
            children_data = sorted(
                children_data, key=lambda x: x.get("update_time"), reverse=True
            )
        if "size" in sorted_fields:
            children_data = sorted(
                children_data, key=lambda x: x.get("size"), reverse=True
            )
        res_data = [
            {
            ...
            if data.get("icon") == "icon-folder":
                ...
                for index, entry in enumerate(data, 1):
                    ...
                    if file_type not in allow_type:
                        continue
                    music_info.update(
                        {"file_full_path": f"{file_full_path}/{each}", "filename": each}
                    )
    ...
    def tidy_folder(self, request, *args, **kwargs):
        ...
        tidy_folder_task(
            music_id3_info,
            {"root_path": root_path, "first_dir": first_dir, "second_dir": second_dir},
        )
        return self.success_response()
...
class TaskModelViewSets(mixins.ListModelMixin, GenericViewSet):
