...
from .encrypt import eEncrypt, linuxEncrypt, weEncrypt
from .public import getCookie, readFile
    ...
    def encrypt(self, data):
        ...
        if self.encrypt_method == "linuxapi":
            ...
            if "MUSIC_U" in cookie:
                data["header"]["MUSIC_U"] = cookie["MUSIC_U"]
            if "MUSIC_A" in cookie:
                data["header"]["MUSIC_A"] = cookie["MUSIC_A"]
            self.headers["Cookie"] = "".join(
                map(lambda key: f"{key}={data['header'][key]};", data["header"])
            )
