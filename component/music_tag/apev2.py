...
class Apev2File(AudioFile):
    ...
    _TAG_MAP = {
        ...
        "albumartist": TAG_MAP_ENTRY(
            getter="Album Artist", setter="Album Artist", type=str
        ),
        "composer": TAG_MAP_ENTRY(getter="Composer", setter="Composer", type=str),
        "tracknumber": TAG_MAP_ENTRY(
            getter=get_tracknum, setter=set_tracknum, remover="Track", type=int
        ),
        "totaltracks": TAG_MAP_ENTRY(
            getter=get_totaltracks, setter=set_totaltracks, remover="Track", type=int
        ),
        "discnumber": TAG_MAP_ENTRY(
            getter=get_discnum, setter=set_discnum, remover="Disc", type=int
        ),
        "totaldiscs": TAG_MAP_ENTRY(
            getter=get_totaldiscs, setter=set_totaldiscs, remover="Disc", type=int
        ),
        ...
        "#codec": TAG_MAP_ENTRY(getter=lambda afile, norm_key: None, type=str),
        "#channels": TAG_MAP_ENTRY(getter=lambda afile, norm_key: None, type=int),
        "#bitspersample": TAG_MAP_ENTRY(getter=lambda afile, norm_key: None, type=int),
        "#samplerate": TAG_MAP_ENTRY(getter=lambda afile, norm_key: None, type=int),
    }
...
class WavePackFile(Apev2File):
    ...
    _TAG_MAP.update(
        {
            "#codec": TAG_MAP_ENTRY(
                getter=lambda afile, norm_key: "WavePack", type=str
            ),
            "#bitrate": TAG_MAP_ENTRY(getter=lambda afile, norm_key: None, type=int),
