...
def set_easy_tracknum(afile, norm_key, val, _tag_name="tracknumber"):
    ...
    tracknumber[0] = val
    afile.set_raw(
        norm_key, _tag_name, "/".join(str(i) for i in tracknumber), appendable=False
    )
...
def set_easy_discnum(afile, norm_key, val, _tag_name="discnumber"):
    ...
    discnumber[0] = val
    afile.set_raw(
        norm_key, _tag_name, "/".join(str(i) for i in discnumber), appendable=False
    )
...
def parse_picture_block(dat):
    ...
    pic = PicBlock(
        typeid=typeid,
        picturetype=PICTURE_TYPE_LUT[typeid],
        mime=mime,
        format=mime.split("/")[1],
        descr=descr,
        width=width,
