    ...
    def uncertified(func):
        ...
        def _wrapper(*args, **kwargs):
            try:
                ...
                raise_tips1 = (
                    f"The function {func.__name__[:-4]}() has been not certified yet."
                )
    ...
    def google_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if reset_host_url and reset_host_url != self.host_url:
            if (
                kwargs.get("if_check_reset_host_url", True)
                and not reset_host_url[:25] == "https://translate.google."
            ):
                raise TranslatorError
        ...
        if self.host_url[-2:] == "cn":
            raise TranslatorError(
                "Google service was offline in inland of China on Oct 2022."
            )

        self.host_headers = self.host_headers or self.get_headers(
            self.host_url, if_api=False
        )
        ...
        if not (
            ...
            self.language_map = self.get_language_map(
                host_html, self.session, timeout, proxies, **debug_lang_kwargs
            )
            ...
            api_url_part_1 = "/translate_a/single?client={0}&sl={1}&tl={2}&hl=zh-CN&dt=at&dt=bd&dt=ex".format(
            ...
            self.api_url = "".join(
                [self.host_url, api_url_part_1, api_url_part_2, api_url_part_3]
            )  # [t,webapp]

        r = self.session.get(
            self.api_url, headers=self.host_headers, timeout=timeout, proxies=proxies
        )
        r.raise_for_status()
        data = r.json()
        ...
        return (
            data
            if is_detail_result
            else "".join([item[0] for item in data[0] if isinstance(item[0], str)])
        )
...
class GoogleV2(Tse):
    ...
    def get_info(self, host_html: str) -> dict:
        data_str = re.compile(r"window.WIZ_global_data = (.*?);</script>").findall(
            host_html
        )[0]
    ...
    def get_consent_cookie(
        self, consent_html: str
    ) -> str:  # by mercuree. merged but not verify.
    ...
    def google_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        self.api_headers = self.get_headers(
            self.host_url, if_api=True, if_referer_for_host=True, if_ajax_for_api=True
        )
        ...
        return (
            {"data": data}
            if is_detail_result
            else " ".join([x[0] for x in (data[1][0][0][5] or data[1][0]) if x[0]])
        )
...
class BaiduV2(Tse):
    def __init__(self):
        ...
        self.acs_url = "https://dlswbr.baidu.com/heicha/mm/{i}/acs-{i}.js".format(
            i=2060
        )
        self.host_headers = self.get_headers(self.host_url, if_api=False)
        self.api_headers = self.get_headers(self.host_url, if_api=True)
    ...
    def get_sign(
        ...
        sign_html = ss.get(
            self.get_sign_url, headers=headers, timeout=timeout, proxies=proxies
        ).text
        begin_label = (
        ...
        sign_js = sign_html[
            sign_html.find(begin_label) + len(begin_label) : sign_html.find(end_label)
        ]
    ...
    def get_tk(self, host_html: str) -> str:
        tk_list = re.compile("""token: '(.*?)',|token: "(.*?)",""").findall(host_html)[
            0
        ]
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.sign = self.get_sign(
                query_text, host_html, self.session, self.host_headers, timeout, proxies
            )
...
class YoudaoV1(Tse):
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        lang_list = sorted(
            [it["code"] for it in data["data"]["value"]["textTranslate"]["specify"]]
        )
        return {}.fromkeys(lang_list, lang_list)

    def get_sign_key(
        self, host_html: str, ss: SessionType, timeout: float, proxies: dict
    ) -> str:
        try:
            if not self.get_sign_url:
                self.get_sign_url = (
                    re.compile(self.get_sign_pattern).search(host_html).group()
                )
            r = ss.get(
                self.get_sign_url,
                headers=self.host_headers,
            ...
            r = ss.get(
                self.get_sign_old_url,
                headers=self.host_headers,
                timeout=timeout,
                proxies=proxies,
            )
            r.raise_for_status()
        sign = re.compile('md5\\("fanyideskweb" \\+ e \\+ i \\+ "(.*?)"\\)').findall(
            r.text
        )
        return sign[0] if sign and sign != [""] else "Ygy_4c=r#e#4EX^NUGUc5"  # v1.1.10

    def get_form(
        self, query_text: str, from_language: str, to_language: str, sign_key: str
    ) -> dict:
...
class YoudaoV2(Tse):
    def __init__(self):
        ...
        self.language_url = "https://api-overmind.youdao.com/openapi/get/luna/dict/luna-front/prod/langType"
        self.domain_url = (
            "https://doctrans-service.youdao.com/common/enums/list?key=domain"
        )
    ...
    def get_default_key(self, js_html: str) -> str:
        return (
            re.compile('="webfanyi-key-getter",(\\w+)="(\\w+)";')
            .search(js_html)
            .group(2)
        )
    ...
    def youdao_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.professional_field_map = self.session.get(
                self.domain_url,
                headers=self.host_headers,
                timeout=timeout,
                proxies=proxies,
            ...
            self.get_js_url = "".join(
                [
                    self.host_url,
                    "/",
                    re.compile(self.get_js_pattern).search(host_html).group(),
                ]
            )
            js_html = self.session.get(
                self.get_js_url,
                headers=self.host_headers,
                timeout=timeout,
                proxies=proxies,
...
class YoudaoV3(Tse):
    ...
    def get_language_map(self, host_html: str, **kwargs: LangMapKwargsType) -> dict:
        ...
        lang_list = sorted(
            [it.split("2")[1] for it in lang_list if f"{self.output_zh}2" in it]
        )
        return {
            **{lang: [self.output_zh] for lang in lang_list},
            **{self.output_zh: lang_list},
        }
...
class QQFanyi(Tse):
    def __init__(self):
        ...
        self.host_headers = self.get_headers(self.host_url, if_api=False)
        self.api_headers = self.get_headers(self.host_url, if_api=True)
        self.qt_headers = self.get_headers(
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        r = ss.get(
            language_url, headers=self.host_headers, timeout=timeout, proxies=proxies
        )
        r.raise_for_status()
        lang_map_str = (
            re.compile("C={(.*?)}|languagePair = {(.*?)}", flags=re.S)
            .search(r.text)
            .group()
        )  # C=
    ...
    def qqFanyi_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        return (
            data
            if is_detail_result
            else "".join(item["targetText"] for item in data["translate"]["records"])
        )  # auto whitespace
...
class QQTranSmart(Tse):
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        js_html = ss.get(
            lang_url, headers=self.host_headers, timeout=timeout, proxies=proxies
        ).text
        lang_str_list = re.compile("lngs:\\[(.*?)]").findall(
            js_html
        )  # 'lngs:\\[(.*?)\\]'
    ...
    def qqTranSmart_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.language_map = self.get_language_map(
                self.get_lang_url, self.session, timeout, proxies, **debug_lang_kwargs
            )

        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "qqTranSmart", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
        )
        ...
        **kwargs: LangMapKwargsType,
        ...
        return dict(
            map(
                lambda x: x,
                [
                    (x["sourceLuange"], x["targetLanguages"])
                    for x in language_dict["languageMap"]
                ],
            )
        )
...
class Bing(Tse):
    ...
    def bing_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        api_url_param = (
            f'?isVertical=1&&IG={self.ig_iid["ig"]}&IID={self.ig_iid["iid"]}'
        )
...
class Sogou(Tse):
    def __init__(self):
        ...
        self.get_language_old_url = (
            "https://search.sogoucdn.com/translate/pc/static/js/app.7016e0df.js"
        )
        self.get_language_pattern = (
            "//search.sogoucdn.com/translate/pc/static/js/vendors.(.*?).js"
        )
        self.get_language_url = None
        self.host_headers = self.get_headers(self.host_url, if_api=False)
        self.api_headers = self.get_headers(self.host_url, if_api=True)
        ...
        **kwargs: LangMapKwargsType,
        ...
        try:
            if not self.get_language_url:
                lang_url_path = (
                    re.compile(self.get_language_pattern).search(host_html).group()
                )
    ...
    def get_form(
        self, query_text: str, from_language: str, to_language: str, uid: str
    ) -> dict:
        sign_text = (
...
class Caiyun(Tse):
    def __init__(self):
        ...
        self.host_headers = self.get_headers(
            self.host_url, if_api=False, if_referer_for_host=True
        )
        self.api_headers = self.get_headers(
    ...
    def decrypt(self, cipher_text: str) -> str:
        _ciphertext = "".join(
            list(map(lambda k: self.decrypt_dictionary[k], cipher_text))
        )
        return base64.b64decode(_ciphertext).decode()
    ...
    def caiyun_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            jwt_r = self.session.post(
                self.get_jwt_url,
                json=self.browser_data,
                headers=self.api_headers,
                timeout=timeout,
                proxies=proxies,
            )
...
class Deepl(Tse):
    ...
    def get_language_map(self, host_html: str, **kwargs: LangMapKwargsType) -> dict:
        lang_list = list(
            set(re.compile('translateIntoLang\\.(\\w+)":').findall(host_html))
        )
        return {}.fromkeys(lang_list, lang_list)
    ...
    def context_sentences_param(
        self, sentences: List[str], from_language: str, to_language: str
    ) -> dict:
        sentences = [""] + sentences + [""]
        data = {
...
class Yandex(Tse):
    def __init__(self):
        ...
        self.api_headers = self.get_headers(self.host_url, if_api=True)
        self.api_headers.update(
            {"Referer": self.api_host, "x-retpath-y": self.host_url}
        )
        self.language_map = None
        self.session = None
        self.sid = None
    ...
    def get_language_map(self, host_html: str, **kwargs: LangMapKwargsType) -> dict:
        lang_str = (
            re.compile("TRANSLATOR_LANGS: {(.*?)},").search(host_html).group(0)[18:-1]
        )
    ...
    def yandex_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if reset_host_url and reset_host_url != self.host_url:
            if (
                kwargs.get("if_check_reset_host_url", True)
                and not reset_host_url[:25] == "https://translate.yandex."
            ):
                raise TranslatorError
            self.host_url = reset_host_url.strip("/")
        ...
        if not (
            ...
            self.yu = (
                self.session.cookies.get_dict().get("yuidss")
                or f"{random.randint(int(1e8), int(9e8))}{int(time.time())}"
            )
        ...
        if from_language == "auto":
            from_language = self.detect_language(
                self.session,
                query_text,
                self.sid,
                self.yu,
...
class Argos(Tse):
    def __init__(self):
        ...
        self.host_headers = self.get_headers(
            self.host_url, if_api=False, if_ajax_for_api=False
        )
        self.api_headers = self.get_headers(
            self.host_url, if_api=True, if_ajax_for_api=False, if_json_for_api=True
        )
        self.language_headers = self.get_headers(
            self.host_url, if_api=False, if_json_for_api=True
        )
        self.host_pool = [
...
class Iciba(Tse):
    def __init__(self):
        ...
        self.api_headers = self.get_headers(
            self.host_url, if_api=True, if_ajax_for_api=True, if_json_for_api=False
        )
        self.language_headers = self.get_headers(
            self.host_url, if_api=False, if_json_for_api=True
        )
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        dd = ss.get(
            api_url, params=params, headers=headers, timeout=timeout, proxies=proxies
        ).json()
    ...
    def iciba_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        sign = hashlib.md5(
            f"6key_web_fanyi{self.s_y2}{query_text}".encode()
        ).hexdigest()[
            :16
        ]  # strip()
        params = {
...
class IflytekV1(Tse):
        ...
        **kwargs: LangMapKwargsType,
        ...
        try:
            if not self.language_url:
                url_path = (
                    re.compile(self.language_url_pattern).search(host_html).group()
                )
                self.language_url = f"{self.host_url[:21]}{url_path}"
            r = ss.get(
                self.language_url, headers=headers, timeout=timeout, proxies=proxies
            )
        except:
            r = ss.get(
                self.language_old_url, headers=headers, timeout=timeout, proxies=proxies
            )
    ...
    def iflytek_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "iflytek", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
        )
        ...
        return (
            data
            if is_detail_result
            else json.loads(data["data"])["trans_result"]["dst"]
        )
...
class IflytekV2(Tse):
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        et = lxml.etree.HTML(host_html)
        host_js_url = (
            f"""{host_true_url}{et.xpath('//script[@type="module"]/@src')[0]}"""
        )
        host_js_html = ss.get(
            host_js_url, headers=headers, timeout=timeout, proxies=proxies
        ).text
    ...
    def iflytek_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            ...
            from_language = (
                detect_r.json()["data"]
                if detect_r.status_code == 200 and detect_r.text.strip() != ""
                else self.output_zh
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
...
class Iflyrec(Tse):
    def __init__(self):
        ...
        self.api_url = (
            "https://fanyi.iflyrec.com/TranslationService/v1/textAutoTranslation"
        )
        self.detect_lang_url = (
            "https://fanyi.iflyrec.com/TranslationService/v1/languageDetection"
        )
        self.language_url = (
            "https://fanyi.iflyrec.com/TranslationService/v1/textTranslation/languages"
        )
        self.host_headers = self.get_headers(self.host_url, if_api=False)
        self.api_headers = self.get_headers(
            self.host_url, if_api=True, if_json_for_api=True
    ...
    def iflyrec_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.language_map = self.get_language_map(
                self.lang_index, **debug_lang_kwargs
            )

        if from_language == "auto":
            params = {"t": self.get_timestamp()}
            form = {"originalText": query_text}
            detect_r = self.session.post(
                self.detect_lang_url,
                params=params,
                json=form,
                headers=self.api_headers,
...
class Reverso(Tse):
    def __init__(self):
        ...
        self.language_pattern = (
            "https://cdn.reverso.net/trans/v(\\d).(\\d).(\\d)/main.js"
        )
        self.host_headers = self.get_headers(self.host_url, if_api=False)
        self.api_headers = self.get_headers(
            self.host_url, if_api=True, if_json_for_api=True
    ...
    def reverso_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.language_url = (
                re.compile(self.language_pattern).search(host_html).group()
            )
            lang_html = self.session.get(
                self.language_url,
                headers=self.host_headers,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "reverso", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
        )
        from_language, to_language = (
            self.decrypt_language_map[from_language],
            self.decrypt_language_map[to_language],
        )
    ...
    def itranslate_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            if not self.language_url:
                manifest_data = self.session.get(
                    self.manifest_url,
                    headers=self.host_headers,
                    timeout=timeout,
                    proxies=proxies,
    ...
    def translateCom_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.language_map = self.get_language_map(
                self.language_description, **debug_lang_kwargs
            )
...
class Utibet(Tse):
    ...
    def utibet_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "utibet", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
        )
        ...
        return (
            {"data_html": data_html}
            if is_detail_result
            else self.parse_result(data_html)
        )
...
class Papago(Tse):
    ...
    def get_authorization(
        self, url: str, auth_key: str, device_id: str, timestamp: int
    ) -> str:
        auth = hmac.new(
            key=auth_key.encode(),
            msg=f"{device_id}\n{url}\n{timestamp}".encode(),
    ...
    def papago_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        detect_auth = self.get_authorization(
            self.lang_detect_url, self.auth_key, self.device_id, detect_time
        )
        detect_add_headers = {
            "device-type": "pc",
            "timestamp": str(detect_time),
            "authorization": detect_auth,
        }
        ...
        trans_auth = self.get_authorization(
            self.api_url, self.auth_key, self.device_id, trans_time
        )
        trans_update_headers = {
            "x-apigw-partnerid": "papago",
            "timestamp": str(trans_time),
...
class Lingvanex(Tse):
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        detail_lang_map = ss.get(
            lang_url, params=params, headers=headers, timeout=timeout, proxies=proxies
        ).json()
        for _ in range(3):
            _ = ss.get(
                lang_url,
        ...
        lang_list = sorted(
            set([item["full_code"] for item in detail_lang_map["result"]])
        )
        return {}.fromkeys(lang_list, lang_list)
    ...
    def get_auth(
        ...
        js_html = ss.get(
            auth_url, headers=headers, timeout=timeout, proxies=proxies
        ).text
    ...
    def lingvanex_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.auth_info = self.get_auth(
                self.auth_url, self.session, self.host_headers, timeout, proxies
            )
            ...
            if mode != self.mode:
                ...
                self.api_url = "".join(
                    [
                        self.auth_info[f"{mode}_BASE_URL"],
                        self.auth_info["TRANSLATE_URL"],
                    ]
                )
                self.language_url = "".join(
                    [
                        self.auth_info[f"{mode}_BASE_URL"],
                        self.auth_info["GET_LANGUAGES_URL"],
                    ]
                )
                self.host_headers.update(
                    {"authorization": self.auth_info[f"{mode}_AUTH_TOKEN"]}
                )
                self.api_headers.update(
                    {"authorization": self.auth_info[f"{mode}_AUTH_TOKEN"]}
                )
            ...
            self.detail_language_map = self.get_d_lang_map(
                self.language_url, self.session, self.host_headers, timeout, proxies
            )

        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "lingvanex", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language,
            to_language,
            self.language_map,
            output_zh=self.output_zh,
            output_en_translator="lingvanex",
            output_en="en_GB",
        )
...
class Mglip(Tse):
    ...
    def mglip_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "mglip", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
        )
        ...
        return (
            ...
            if is_detail_result
                ...
                data["datas"][0]["paragraph"]
                if data["datas"][0]["type"] == "trans"
                else data["datas"][0]["data"]
            )
        )
...
class VolcEngine(Tse):
    ...
    def professional_field_map(self) -> dict:
        data = {
            ...
            "ecommerce": {
                "category": "ecommerce",
                "glossary_list": ["ailab/ecommerce"],
            },
            "technique": {
                "category": "technique",
                "glossary_list": ["ailab/technique"],
            },
        }
        return data
...
class ModernMt(Tse):
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        d_lang_map = json.loads(
            re.compile("""('{(.*?)}')""").search(lang_html).group(0)[1:-1]
        )
    ...
    def modernMt_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        payload = {
            ...
            "ts": timestamp,
            "verify": hashlib.md5(
                f"webkey_E3sTuMjpP8Jez49GcYpDVH7r#{timestamp}#{query_text}".encode()
            ).hexdigest(),
            "hints": "",
            "multiline": "true",
        }
...
class MyMemory(Tse):
    ...
    def get_language_map(
        self,
        myMemory_host_html: str,
        matecat_lang_url: str,
        ss: SessionType,
        headers: dict,
        timeout: Optional[float],
        proxies: Optional[dict],
        **kwargs: LangMapKwargsType,
        ...
        lang_d_list = ss.get(
            matecat_lang_url, headers=headers, timeout=timeout, proxies=proxies
        ).json()
        self.mateCat_language_list = sorted(
            list(set([item["code"] for item in lang_d_list]))
        )

        lang_list = sorted(
            list(set(self.myMemory_language_list + self.mateCat_language_list))
        )
        return {}.fromkeys(lang_list, lang_list)
    ...
    def myMemory_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "myMemory", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language,
            to_language,
            self.language_map,
            output_zh=self.output_zh,
            output_en_translator="myMemory",
            output_en="en-GB",
        )

        params = {"q": query_text, "langpair": f"{from_language}|{to_language}"}
...
class Mirai(Tse):
    def __init__(self):
        ...
        self.detect_lang_url = (
            "https://trial.miraitranslate.com/trial/api/detect_lang.php"
        )
        ...
        self.api_json_headers = self.get_headers(
            self.home_url, if_api=True, if_json_for_api=True
        )
        self.api_text_headers = self.get_headers(
            self.home_url, if_api=True, if_ajax_for_api=False
        )
        self.session = None
        self.language_map = None
        self.tran_key = None
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        return {
            f_lang: [v for k, v in lang_pairs if k == f_lang]
            for f_lang, t_lang in lang_pairs
        }
    ...
    def mirai_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            ...
            from_language = (
                self.lang_zh_map[from_language]
                if "zh" in from_language
                else from_language
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, output_zh=self.output_zh
        ...
        self.query_count += 1
        return (
            data if is_detail_result else data["ouputs"][0]["output"][0]["translation"]
        )
...
class Tilde(Tse):
    def __init__(self):
        ...
        self.get_config_url = (
            "https://translate.tilde.com/assets/config.local.json"  # ?version=46852
        )
        self.subscribe_url = (
            "https://translate.tilde.com/assets/subscriptions-config.local.json"
        )
    ...
    def get_language_map(self, sys_data: dict, **kwargs: LangMapKwargsType) -> dict:
        lang_pairs = [
            [item["SourceLanguage"]["Code"], item["TargetLanguage"]["Code"]]
            for item in sys_data["System"]
            if "General" in item["Domain"]
        ]
        return {
            f_lang: [v for k, v in lang_pairs if k == f_lang]
            for f_lang, t_lang in lang_pairs
    ...
    def get_langpair_ids(self, sys_data: dict) -> dict:
        return {
            f"{item['SourceLanguage']['Code']}-{item['TargetLanguage']['Code']}": item[
                "ID"
            ]
            for item in sys_data["System"]
            if "General" in item["Domain"]
    ...
    def tilde_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.config_data = self.session.get(
                self.get_config_url,
                headers=self.host_headers,
                timeout=timeout,
                proxies=proxies,
            ...
            self.api_headers.update(
                {"client-id": self.config_data["mt"]["api"]["clientId"]}
            )  # must lower keyword
            ...
            self.sys_data = self.session.get(
                sys_url,
                params=params,
                headers=self.api_headers,
                timeout=timeout,
            ...
            self.language_map = self.get_language_map(
                self.sys_data, **debug_lang_kwargs
            )

        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "tilde", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
...
class CloudYi(Tse):
    def __init__(self):
        ...
        self.api_url = (
            "https://www.cloudtranslation.com/official-website/v1/transOneSrcText"
        )
        self.get_lang_url = "https://online.cloudtranslation.com/api/v1.0/site/get_all_language_and_domain"
        self.detect_lang_url = (
            "https://online.cloudtranslation.com/api/v1.0/request_translate/langid"
        )
        self.get_cookie_url = (
            "https://online.cloudtranslation.com/api/v1.0/site/sites_language_list"
        )
        self.host_headers = self.get_headers(
            self.home_url, if_api=False, if_referer_for_host=True
        )
        self.api_headers = self.get_headers(
            self.home_url, if_api=True, if_json_for_api=True
        )
    ...
    def get_language_map(self, d_lang_map: dict, **kwargs: LangMapKwargsType) -> dict:
        return {
            k: [it["language_code"] for it in item]
            for k, item in d_lang_map["data"]["src_to_tgt"].items()
        }

    def get_langpair_domain(self, d_lang_map: dict) -> dict:
        return {
            k: [it["domain_code"] for it in item]
            for k, item in d_lang_map["data"]["language_pair_to_domain"].items()
        }

    def get_professional_field_list(self, d_lang_map: dict) -> set:
        return {
            it["domain_code"]
            for _, item in d_lang_map["data"]["language_pair_to_domain"].items()
            for it in item
        }
...
class SysTran(Tse):
    def __init__(self):
        ...
        self.api_ajax_headers = self.get_headers(
            self.home_url, if_api=True, if_ajax_for_api=True
        )
        self.api_json_headers = self.get_headers(
            self.home_url, if_api=True, if_json_for_api=True
        )
    ...
    def get_language_map(self, d_lang_map: dict, **kwargs: LangMapKwargsType) -> dict:
        return {
            ii["source"]: [
                jj["target"]
                for jj in d_lang_map["languagePairs"]
                if jj["source"] == ii["source"]
            ]
            for ii in d_lang_map["languagePairs"]
        }

    def get_professional_field_list(self, d_lang_map: dict) -> set:
        return {
            it["selectors"]["domain"]
            for item in d_lang_map["languagePairs"]
            for it in item["profiles"]
        }
    ...
    def get_client_data(
        ...
        js_html = ss.get(
            client_url, headers=headers, timeout=timeout, proxies=proxies
        ).text
        search_groups = re.compile(
            '"https://translate.systran.net/oidc",\\w="(.*?)",\\w="(.*?)";'
        ).search(
            js_html
        )  # \\w{1} == \\w
        client_data = {
    ...
    def sysTran_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.client_data = self.get_client_data(
                self.get_client_url, self.session, self.host_headers, timeout, proxies
            )
            payload = urllib.parse.urlencode(self.client_data)
            self.token_data = self.session.post(
                self.get_token_url,
                data=payload,
                headers=self.api_ajax_headers,
                timeout=timeout,
                proxies=proxies,
            ).json()
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "sysTran", self.default_from_language, if_print_warning
            )
        ...
        if use_domain and from_language != "auto":
            domain_payload = self.langpair_domain.get(
                f"{from_language}__{to_language}__{use_domain}"
            )
        ...
        return (
            ...
            if is_detail_result
                ...
                " ".join(
                    it["alt_transes"][0]["target"]["text"]
                    for it in item["output"]["documents"][0]["trans_units"][0][
                        "sentences"
                    ]
                )
...
class TranslateMe(Tse):
    def __init__(self):
        ...
        self.api_headers = self.get_headers(
            self.host_url, if_api=True, if_ajax_for_api=True
        )
        self.session = None
        self.language_map = None
        self.query_count = 0
    ...
    def _translateMe_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "translateMe", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language,
            to_language,
            self.language_map,
            output_zh=self.output_zh,
            output_en_translator="translateMe",
            output_en=self.output_en,
        )
    ...
    def translateMe_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if self.output_en in (from_language, to_language):
            return self._translateMe_api(
                query_text, from_language, to_language, **kwargs
            )

        tmp_kwargs = kwargs.copy()
        tmp_kwargs.update({"is_detail_result": False, "if_show_time_stat": False})
        next_query_text = self._translateMe_api(
            query_text, from_language, self.output_en, **tmp_kwargs
        )
        return self._translateMe_api(
            next_query_text, self.output_en, to_language, **kwargs
        )
...
class Elia(Tse):
    ...
    def get_language_map(self, dd: dict, **kwargs: LangMapKwargsType) -> dict:
        return {
            ii["source_language"]["code"]: [
                jj["target_language"]["code"]
                for jj in dd["language_pairs"]
                if jj["source_language"]["code"] == ii["source_language"]["code"]
            ]
            for ii in dd["language_pairs"]
        }
    ...
    def elia_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.token = (
                re.compile('"csrfmiddlewaretoken": "(.*?)"').search(host_html).group(1)
            )
            d_lang_str = (
                re.compile("var languagePairs = JSON.parse\\((.*?)\\);")
                .search(host_html)
                .group()
            )
...
class LanguageWire(Tse):
    def __init__(self):
        ...
        self.lang_url = (
            "https://lwt.languagewire.com/f/api/v1/language-pairs?includeVariants=true"
        )
    ...
    def get_language_map(
        ...
        **kwargs: LangMapKwargsType,
        ...
        return {
            ii["sourceLanguage"]["mmtCode"]: [
                jj["targetLanguage"]["mmtCode"]
                for jj in d_lang_map
                if jj["sourceLanguage"]["mmtCode"] == ii["sourceLanguage"]["mmtCode"]
            ]
    ...
    def languageWire_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "languageWire", self.default_from_language, if_print_warning
            )
        to_language = (
            self.default_en_to_language if to_language == "en" else to_language
        )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map, if_check_lang_reverse=False
        )
...
class Judic(Tse):
    ...
    @Tse.debug_language_map
    def get_language_map(
        self, lang_list: List[str], **kwargs: LangMapKwargsType
    ) -> dict:
        return {}.fromkeys(lang_list, lang_list)
    ...
    def judic_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if not (
            ...
            self.language_map = self.get_language_map(
                self.lang_list, **debug_lang_kwargs
            )

        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "judic", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map
        )
...
class Yeekit(Tse):
    ...
    def yeekit_api(
        ...
        **kwargs: ApiKwargsType,
        ...
        if from_language == "auto":
            from_language = self.warning_auto_lang(
                "yeekit", self.default_from_language, if_print_warning
            )
        from_language, to_language = self.check_language(
            from_language, to_language, self.language_map
        )
        ...
        return (
            ...
            if is_detail_result
                ...
                " ".join(p)
                for p in json.loads(data[0])["translation"][0]["translated"][0][
                    "translation list"
                ]
            )
        )
...
class TranslatorsServer:
    def __init__(self):
        ...
        self._translators_dict = {
            ...
            "apertium": self._apertium,
            "argos": self._argos,
            "baidu": self._baidu,
            "bing": self._bing,
            "caiyun": self._caiyun,
            "cloudYi": self._cloudYi,
            "deepl": self._deepl,
            "elia": self._elia,
            "google": self._google,
            "iciba": self._iciba,
            "iflytek": self._iflytek,
            "iflyrec": self._iflyrec,
            "itranslate": self._itranslate,
            "judic": self._judic,
            "languageWire": self._languageWire,
            "lingvanex": self._lingvanex,
            "mglip": self._mglip,
            "mirai": self._mirai,
            "modernMt": self._modernMt,
            "myMemory": self._myMemory,
            "papago": self._papago,
            "qqFanyi": self._qqFanyi,
            "qqTranSmart": self._qqTranSmart,
            "reverso": self._reverso,
            "sogou": self._sogou,
            "sysTran": self._sysTran,
            "tilde": self._tilde,
            "translateCom": self._translateCom,
            "translateMe": self._translateMe,
            "utibet": self._utibet,
        ...
        self.translators_dict = {
            ...
            "apertium": self.apertium,
            "argos": self.argos,
            "baidu": self.baidu,
            "bing": self.bing,
            "caiyun": self.caiyun,
            "cloudYi": self.cloudYi,
            "deepl": self.deepl,
            "elia": self.elia,
            "google": self.google,
            "iciba": self.iciba,
            "iflytek": self.iflytek,
            "iflyrec": self.iflyrec,
            "itranslate": self.itranslate,
            "judic": self.judic,
            "languageWire": self.languageWire,
            "lingvanex": self.lingvanex,
            "mglip": self.mglip,
            "mirai": self.mirai,
            "modernMt": self.modernMt,
            ...
            "sogou": self.sogou,
            "sysTran": self.sysTran,
            "tilde": self.tilde,
            "translateCom": self.translateCom,
            "translateMe": self.translateMe,
            "utibet": self.utibet,
    ...
    def translate_html(
        ...
        **kwargs: ApiKwargsType,
        ...
        if (
            translator not in self.translators_pool
            or kwargs.get("is_detail_result", False)
            or n_jobs > self.cpu_cnt
        ):
            raise TranslatorError
        ...
        def _translate_text(sentence: str) -> Tuple[str, str]:
            return sentence, self.translators_dict[translator](
                query_text=sentence,
                from_language=from_language,
                to_language=to_language,
                **kwargs,
        ...
        pattern = re.compile(
            "(?:^|(?<=>))([\\s\\S]*?)(?:(?=<)|$)"
        )  # TODO: <code></code> <div class="codetext notranslate">
    ...
    def _test_translate(
        self, _ts: str, timeout: Optional[float] = None, if_show_time_stat: bool = False
    ) -> str:
    ...
    def preaccelerate(
        ...
        if_show_time_stat: bool = True,
        **kwargs: str,
    ) -> dict:
        if self.pre_acceleration_label > 0:
            raise TranslatorError("Preacceleration can only be performed once.")

        self.example_query_text = kwargs.get(
            "example_query_text", self.example_query_text
        )
        ...
        for i in tqdm.tqdm(
            range(len(self.translators_pool)), desc="Preacceleration Process", ncols=80
        ):
        ...
        return {
            "success": self.success_translators_pool,
            "failure": self.failure_translators_pool,
        }
    ...
    def speedtest(self, **kwargs: List[str]) -> None:
        ...
        test_translators_pool = kwargs.get(
            "test_translators_pool", self.success_translators_pool
        )

        sys.stderr.write("SpeedTest-Process will take a few seconds.\n\n")
        for i in tqdm.tqdm(
            range(len(test_translators_pool)), desc="SpeedTest Process", ncols=80
        ):
    ...
    def preaccelerate_and_speedtest(
        self, timeout: Optional[float] = None, **kwargs: str
    ) -> dict:
