  ...
  <div style="display: flex; flex-wrap: wrap">
          ...
          <div style="margin-left: 10px; margin-right: 5px">
            <bk-dropdown-menu :align="'right'">
              <template slot="dropdown-trigger">
                <span
                  class="dropdown-trigger-btn bk-icon icon-sort"
              ...
              <ul class="bk-dropdown-list" slot="dropdown-content">
                <li>
                  <a
                    href="javascript:;"
                    @click="changeSorted('name')"
                    :class="{ isSelected: sortedField.includes('name') }"
                    >名称</a
                  >
                </li>
                <li>
                  <a
                    href="javascript:;"
                    @click="changeSorted('update_time')"
                    :class="{ isSelected: sortedField.includes('update_time') }"
                    >修改时间</a
                  >
                </li>
                <li>
                  <a
                    href="javascript:;"
                    @click="changeSorted('size')"
                    :class="{ isSelected: sortedField.includes('size') }"
                    >大小</a
                  >
                </li>
              </ul>
    ...
    <div class="edit-section">
      <transition name="bk-slide-fade-left">
        <div
          style="margin-left: 40px; width: 500px; margin-top: 20px"
        ...
        >
          <div style="width: 100%; display: flex; align-items: center">
            ...
            <div
              ...
              @click="exampleSetting3.primary.visible = true"
            >
          ...
          >
            <div
              class="label1 can-copy"
              v-bk-tooltips="'变量名:${title}'"
              v-bk-copy="'${title}'"
            >
              标题：
            </div>
          ...
          <div v-for="(item, index) in showFields" :key="'l1' + index">
            ...
            <div class="edit-item can-copy" v-else-if="item === 'albumartist'">
              <div
                class="label1"
                v-bk-tooltips="'变量名:${albumartist}'"
                v-bk-copy="'${albumartist}'"
              >
                专辑艺术家：
              </div>
              <div style="width: 70%">
            ...
            <div
              style="display: flex; margin-bottom: 10px; flex-direction: column"
            ...
            >
              ...
              <div style="display: flex; margin-top: 10px">
                ...
                <bk-switcher
                  v-model="musicInfo.is_save_lyrics_file"
                ></bk-switcher>
              </div>
            </div>
            ...
            <div class="edit-item" v-else-if="item === 'album_img'">
              ...
              <div style="display: flex; flex-direction: column">
                ...
                <div style="display: flex; margin-top: 10px">
                  ...
                  <bk-switcher
                    v-model="musicInfo.is_save_album_cover"
                  ></bk-switcher>
                </div>
              </div>
            </div>
        ...
        >
          ...
          <div style="width: 100%; display: flex; margin-top: 10px">
            <bk-button
              :theme="'success'"
              :loading="isLoading"
              @click="exampleSetting1.primary.visible = true"
              class="mr10"
              style="width: 50%"
            >
              自动修改
            ...
            <bk-button
              :theme="'success'"
              :loading="isLoading"
              @click="exampleSetting2.primary.visible = true"
              class="mr10"
              style="width: 50%"
            >
              整理文件夹
          ...
          >
            ...
            <div style="width: 70%">
              <bk-input
                :clearable="true"
                v-model="musicInfoManual.title"
                :placeholder="'支持变量批量修改'"
              ></bk-input>
            </div>
          </div>
          <div v-for="(item, index) in showFields" :key="'l2' + index">
            <div class="edit-item" v-if="item === 'filename'">
              ...
              <div style="width: 70%">
                <bk-input
                  :clearable="true"
                  v-model="musicInfoManual.filename"
                  :placeholder="'例如：${title}-${album}'"
                ></bk-input>
              </div>
            </div>
            <div class="edit-item" v-else-if="item === 'artist'">
              ...
              <div style="width: 70%">
                <bk-input
                  :clearable="true"
                  v-model="musicInfoManual.artist"
                  :placeholder="'具体哪些变量,鼠标悬浮在标题上查看'"
                ></bk-input>
              </div>
            </div>
            ...
            <div class="edit-item" v-else-if="item === 'albumartist'">
              ...
              <div style="width: 70%">
                <bk-input
                  :clearable="true"
                  v-model="musicInfoManual.albumartist"
                ></bk-input>
              </div>
            </div>
            <div class="edit-item" v-else-if="item === 'genre'">
            ...
            >
              <div style="display: flex">
                ...
                <div style="width: 70%">
                  <bk-input
                    :clearable="true"
                    v-model="musicInfoManual.lyrics"
                    type="textarea"
                    :rows="15"
                  ></bk-input>
                </div>
              ...
              <div style="display: flex; margin-top: 10px">
                <div class="label1">保存歌词：</div>
                <bk-switcher
                  v-model="musicInfoManual.is_save_lyrics_file"
                ></bk-switcher>
              </div>
            </div>
            <div class="edit-item" v-else-if="item === 'comment'">
              ...
              <div style="width: 70%">
                <bk-input
                  :clearable="true"
                  v-model="musicInfoManual.comment"
                  type="textarea"
                ></bk-input>
              </div>
            </div>
            <div class="edit-item" v-else-if="item === 'album_img'">
              ...
              <div style="display: flex; flex-direction: column">
                ...
                <div style="display: flex; margin-top: 10px">
                  <div class="label1">保存图片：</div>
                  <bk-switcher
                    v-model="musicInfoManual.is_save_album_cover"
                  ></bk-switcher>
                </div>
              </div>
            </div>
            <div class="edit-item" v-else-if="item === 'discnumber'">
              <div class="label1" v-bk-tooltips="'变量名:${discnumber}'">
                光盘编号：
              </div>
              <div style="width: 70%">
                <bk-input
                  :clearable="true"
                  v-model="musicInfoManual.discnumber"
                ></bk-input>
              </div>
            </div>
            <div class="edit-item" v-else-if="item === 'tracknumber'">
              <div class="label1" v-bk-tooltips="'变量名:${tracknumber}'">
                音轨号：
              </div>
              <div style="width: 70%">
                <bk-input
                  :clearable="true"
                  v-model="musicInfoManual.tracknumber"
                ></bk-input>
              </div>
            </div>
          </div>
    ...
    <div class="resource-section">
        ...
        >
          ...
          <div v-else>
            ...
            >
              ...
              <div
                @click="handleCopy('artist', item.artist)"
                class="music-item"
              >
                {{ item.artist }}
              </div>
      ...
      <transition name="bk-slide-fade-left">
        <div v-show="showTranslation">
          <div style="display: flex; height: 100%">
            <bk-icon
              type="arrows-left-shape"
              @click="handleCopy('lyric_tran', translationText)"
              style="
                margin-right: 5px;
                margin-left: 15px;
                margin-top: 50%;
      ...
      <div
        v-show="!fadeShowDetail && !showTranslation"
        style="width: 90%; height: 90%; margin: 50px 20px 20px 50px"
      >
        <bk-image
          fit="contain"
          :src="'/static/dist/img/music_null-cutout.png'"
          style="width: 100%; height: 98%"
        ></bk-image>
      </div>
    </div>
    <bk-dialog
      v-model="exampleSetting1.primary.visible"
      theme="primary"
      :mask-close="false"
    ...
    >
      ...
      <bk-radio-group v-model="selectAutoMode">
        <bk-radio-button value="simple"> 宽松模式 </bk-radio-button>
        <bk-radio-button value="hard"> 严格模式 </bk-radio-button>
      </bk-radio-group>
      <div>音乐源顺序</div>
      <bk-select
        style="width: 250px"
      ...
      >
        <bk-option
          v-for="option in resourceListBatch"
          :key="option.id"
          :id="option.id"
          :name="option.name"
        >
    ...
    <bk-dialog
      v-model="exampleSetting2.primary.visible"
      theme="primary"
      :mask-close="false"
      @confirm="handleTidy"
    ...
    >
      ...
      <div class="input-demo">
        <bk-input v-model="tidyFormData.root_path"> </bk-input>
      </div>
      <div>一级目录</div>
      <bk-select
        style="width: 250px"
...
export default {
  data() {
    return {
      ...
      uploadHeader: [
        {
          name: "X-CSRFToken",
          value: this.getCookie("django_vue_cli_csrftoken"),
        },
        { name: "AUTHORIZATION", value: this.getCookie("AUTHORIZATION") },
      ],
      showFields: localStorage.getItem("showFields")
        ? JSON.parse(localStorage.getItem("showFields"))
        : [
            "filename",
            "artist",
            "album",
      ...
      sortedField: localStorage.getItem("sortedField")
        ? JSON.parse(localStorage.getItem("sortedField"))
        : [],
    };
  },
  computed: {
  ...
  methods: {
      ...
      if (node.title.length > 25) {
        return (
          <span>
            <span
              class={titleClass}
              domPropsInnerHTML={node.title.slice(0, 25)}
              onClick={() => {
    ...
    handleCopy(k, v) {
      if (k === "lyric") {
        const resurce =
          this.resource !== "smart_tag" ? this.resource : v.resource;
        this.$api.Task.fetchLyric({ song_id: v.id, resource: resurce }).then(
          (res) => {
            if (res.result) {
